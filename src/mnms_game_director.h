...
public:
    ...
    // Jump directly to a level by pack id and level index, bypassing the menu
    bool jump_to_level(const String &p_pack_id, int32_t p_level_index);
    void set_skip_menu(bool p_skip) { skip_menu = p_skip; }
    bool get_skip_menu() const { return skip_menu; }
};
