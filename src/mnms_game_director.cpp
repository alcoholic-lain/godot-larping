...
void MnmsGameDirector::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("jump_to_level", "pack_id", "level_index"), &MnmsGameDirector::jump_to_level);
    ClassDB::bind_method(D_METHOD("set_skip_menu", "skip"), &MnmsGameDirector::set_skip_menu);
    ClassDB::bind_method(D_METHOD("get_skip_menu"), &MnmsGameDirector::get_skip_menu);
    ADD_PROPERTY(PropertyInfo(Variant::BOOL, "skip_menu"), "set_skip_menu", "get_skip_menu");
}
...
bool MnmsGameDirector::jump_to_level(const String &p_pack_id, int32_t p_level_index) {
    if (!_load_selector_pack(p_pack_id)) {
        return false;
    }
    if (!_load_pack(p_pack_id)) {
        return false;
    }
    _set_title_visible(false);
    _set_selector_visible(false);
    _set_interlevel_visible(false);
    _set_pause_visible(false);
    return _load_level_at_index(p_level_index, true, false);
}
